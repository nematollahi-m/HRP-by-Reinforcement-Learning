...
class EconomicEnv(Env):
    ...
    def step(self, action):
        ...
        if pl_t >= p_t:
            c_hire = HIRE_COST * (h_b_t + h_i_t + h_a_t)
            c_fire = FIRE_COST * (f_b_t + f_i_t + f_a_t)
            c_wage = ((WAGE_BEG * m_b_t) + (WAGE_INT * m_i_t) + (WAGE_ADV * m_a_t)) * (pl_t / p_t)
            c_t = c_hire + c_fire + c_wage
            ...
            if c_t > self.budget:
                ...
                if c_t == 0:
                    ...
                    #r_t = alpha_economic * ((PLANTS - p_t + pl_t) / PLANTS) * M * 5
                    r_t = (1 / (c_t / BUDGET)) * M
                done = True
                return self.state, r_t, done, info
        ...
        if self.prune_len <= 1:
            ...
            # r_t = alpha_economic * ((PLANTS - p_t + pl_t) / PLANTS)
            r_t = -M
            self.state = np.asarray([m_b_t, m_i_t, m_a_t, 0, p_t - int(pl_t)])
            self.budget = self.budget - c_t
            done = True
            return self.state, r_t, done, info
