...
class FarmEnvSocial(Env):
    ...
    def step(self, action):
        ...
        if self.prune_len <= 1:
            print('End of season')
            #r_t = alpha_social * (h_t / (m_t + 1)) - beta_social * (f_t / (m_t_1 + 1)) + delta_social * ((PLANTS-p_t+pl_t) / PLANTS)
            r_t = -M
            self.state = np.asarray([m_t, b_t - int(c_t), p_t - int(pl_t)])
            done = True
            return self.state, r_t, done, info

        r_t = (h_t / (m_t + 1)) - (f_t / (m_t_1 + 1)) + (pl_t / PLANTS)
        self.state = np.asarray([m_t, b_t - int(c_t), p_t - int(pl_t)])
        self.prune_len -= 1
        return self.state, r_t, done, info
