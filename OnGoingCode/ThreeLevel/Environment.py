...
class EnvironmentEnv(Env):
    ...
    def step(self, action):
        ...
        mapping = tuple(np.ndindex((MAX_ALLOWED_WORKER + 1, MAX_ALLOWED_WORKER + 1, MAX_ALLOWED_WORKER + 1, MAX_ALLOWED_WORKER + 1,
                                    MAX_ALLOWED_WORKER + 1, MAX_ALLOWED_WORKER + 1)))
        ...
        if pl_t >= p_t:
            ...
            if c_t > self.budget:
                ...
                if c_t == 0:
                    ...
                    r_t = ((QUALITY_BEG + QUALITY_INT + QUALITY_ADV) - (c_t/BUDGET)) * M
                # r_t = (alpha_env + beta_env * QUALITY_BEG) * (pl_b_t / PLANTS) + (alpha_env + beta_env * QUALITY_INT) *\
                #       (pl_i_t / PLANTS) + (alpha_env + beta_env * QUALITY_ADV) * (pl_a_t / PLANTS)
        ...
        if self.prune_len <= 1:
            ...
            # r_t = (alpha_env + beta_env * QUALITY_BEG) * (pl_b_t / PLANTS) + (alpha_env + beta_env * QUALITY_INT) * \
            #       (pl_i_t / PLANTS) + (alpha_env + beta_env * QUALITY_ADV) * (pl_a_t / PLANTS)
            self.state = np.asarray([m_b_t, m_i_t, m_a_t, 0, int(p_t - pl_t)])
            self.budget = self.budget - c_t
            done = True
            return self.state, r_t, done, info
        ...
        if c_t == 0 or pl_t == 0:
            ...
            r_t = ((QUALITY_BEG * pl_b_t) + (QUALITY_INT * pl_i_t) + (QUALITY_ADV * pl_a_t)) / PLANTS - (c_t/BUDGET)

        self.state = np.asarray([m_b_t, m_i_t, m_a_t, 0, p_t - pl_t])
        self.prune_len -= 1
        self.budget = self.budget - c_t
