...
class EconomicEnvNew(Env):
    ...
    def step(self, action):
        ...
        # pl_b_t = m_b_t * round(np.random.normal(PRODUCTIVITY_BEG, 0.03448))
        # pl_i_t = m_i_t * round(np.random.normal(PRODUCTIVITY_INT, 0.0258))
        # pl_a_t = m_a_t * round(np.random.normal(PRODUCTIVITY_ADV, 0.01724))
        ...
        if pl_t >= p_t:
            ...
            if c_t > self.budget:
                ...
                if c_t == 0:
                    ...
                    # r_t = (self.budget/c_t) + 50
                    # r_t = (pl_t/PLANTS) * (BUDGET/c_t) + M
                    # r_t = self.budget / BUDGET
                    # r_t = (p_t/PLANTS)-(c_t/BUDGET) + M
                    # r_t = p_t / c_t + M
                    # r_t = self.budget/BUDGET + M
                    r_t = (p_t * Num_acr) / c_t + M*M
        ...
        if self.prune_len <= 1:
            ...
            if c_t == 0:
                ...
                # r_t = (pl_t/self.plants) * (self.budget/c_t)
                # r_t = (pl_t/PLANTS)-(c_t/BUDGET)
                r_t = (pl_t * Num_acr) / c_t
